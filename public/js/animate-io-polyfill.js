 ...
 * 
 * Features requested: Array.from,Array.prototype.forEach,IntersectionObserver,Number.isNaN
 * 
 ...
 * - _ESAbstract.CreateMethodProperty, License: CC0 (required by "Array.from", "Set", "Symbol", "Object.getOwnPropertyNames", "Object.keys")
 * - _ESAbstract.Get, License: CC0 (required by "Array.from", "_ESAbstract.ToString", "_ESAbstract.ToPrimitive", "_ESAbstract.OrdinaryToPrimitive")
 ...
 * - _ESAbstract.ToObject, License: CC0 (required by "Array.from", "Set", "Symbol", "Object.getOwnPropertyDescriptor")
...
(function (self, undefined) {
  ...
  */
  ...
  /* [, PreferredType] */
    ...
    if (Type(input) === 'object') {
      ...
      var exoticToPrim = typeof self.Symbol === 'function' && _typeof2(self.Symbol.toPrimitive) === 'symbol' ? GetMethod(input, self.Symbol.toPrimitive) : undefined; // e. If exoticToPrim is not undefined, then
  ...
  */
  ...
  })(window, document); // Number.isNaN

  /* global CreateMethodProperty, Type */


  (function () {
    var that = self; // 20.1.2.4. Number.isNaN ( number )
  ...
  })(Object.isExtensible); // Object.keys
  ...
  CreateMethodProperty(Object, "keys", function () {
    ...
    var hasAutomationEqualityBug = function () {
      ...
      for (var k in window) {
        try {
          if (!excludedKeys['$' + k] && has.call(window, k) && window[k] !== null && _typeof2(window[k]) === 'object') {
            try {
    ...
    function isArgumentsObject(value) {
      ...
      if (!isArgs) {
        isArgs = str !== '[object Array]' && value !== null && _typeof2(value) === 'object' && typeof value.length === 'number' && value.length >= 0 && toStr.call(value.callee) === '[object Function]';
      }
  ...
  (function () {
    ...
    var cachedWindowNames = _typeof2(self) === "object" ? nativeGetOwnPropertyNames(self) : []; // 19.1.2.10 Object.getOwnPropertyNames ( O )
  ...
  })(Object, 'getOwnPropertySymbols', this); // Symbol.iterator
  ...
  (function (global) {
    ...
    var _uniqueHashId = 0; // Create a unique key name for storing meta data on functions and objects to enable lookups in hash table
    ...
    var hashKey = function hashKey(recordKey) {
      // Check to see if we are dealing with object or function type.
      if (_typeof2(recordKey) === 'object' ? recordKey !== null : typeof recordKey === 'function') {
        ...
        if (!Object.prototype.hasOwnProperty.call(recordKey, _metaKey)) {
          var uniqueHashKey = _typeof2(recordKey) + '-' + ++_uniqueHashId;
    ...
    var getRecordIndex = function getRecordIndex(map, recordKey) {
    ...
    var getRecordIndexSlow = function getRecordIndexSlow(map, recordKey) {
      // We have to iterate through our Map structure because `recordKey` is non-primitive and not extensible
    ...
    var setHashIndex = function setHashIndex(map, recordKey, recordIndex) {
    ...
    /* iterable */
    {
      ...
      var map = OrdinaryCreateFromConstructor(this, Map.prototype, {
        _table: {},
        // O(1) access table for retrieving records
        _keys: [],
        _values: [],
        _size: 0,
        _es6Map: true
    ...
    CreateMethodProperty(Map.prototype, 'clear', function clear() {
      ...
      this._table = {}; // 6. Return undefined.
    ...
    CreateMethodProperty(Map.prototype, 'delete', function (key) {
      ...
      if (recordIndex !== false) {
        ...
        var recordKey = M._keys[recordIndex]; // 5a. If p.[[Key]] is not empty and SameValueZero(p.[[Key]], key) is true, then

        if (recordKey !== undefMarker && SameValueZero(recordKey, key)) {
          // i. Set p.[[Key]] to empty.
          this._keys[recordIndex] = undefMarker; // ii. Set p.[[Value]] to empty.
          ...
          setHashIndex(this, key, false); // iii. Return true.
  ...
  */
  ...
  (function () {
    ...
    var stringMatch = String.prototype.match; // A cross-realm friendly way to detect if a value is a String object or literal.
    ...
    /* [ , mapfn [ , thisArg ] ] */
      ...
      var usingIterator = GetMethod(items, Symbol.iterator); // 5. If usingIterator is not undefined, then

      if (usingIterator !== undefined) {
        // a. If IsConstructor(C) is true, then
        if (IsConstructor(C)) {
          // i. Let A be ? Construct(C).
          var A = Construct(C); // b. Else,
        } else {
        ...
        var iteratorRecord = GetIterator(items, usingIterator); // d. Let k be 0.
        ...
        while (true) {
          ...
          if (k >= Math.pow(2, 53) - 1) {
            ...
            var error = new TypeError('Iteration count can not be greater than or equal 9007199254740991.'); // 2. Return ? IteratorClose(iteratorRecord, error).

            return IteratorClose(iteratorRecord, error);
          } // ii. Let Pk be ! ToString(k).


          var Pk = ToString(k); // iii. Let next be ? IteratorStep(iteratorRecord).

          var next = IteratorStep(iteratorRecord); // iv. If next is false, then

          if (next === false) {
            // 1. Perform ? Set(A, "length", k, true).
            A.length = k; // 2. Return A.

            return A;
          } // v. Let nextValue be ? IteratorValue(next).


          var nextValue = IteratorValue(next); // vi. If mapping is true, then

          if (mapping) {
            try {
              // Polyfill.io - The try catch accounts for step 2.
              // 1. Let mappedValue be Call(mapfn, T, « nextValue, k »).
              var mappedValue = Call(mapfn, T, [nextValue, k]); // 2. If mappedValue is an abrupt completion, return ? IteratorClose(iteratorRecord, mappedValue).
          ...
          try {
            ...
            CreateDataPropertyOrThrow(A, Pk, mappedValue); // ix. If defineStatus is an abrupt completion, return ? IteratorClose(iteratorRecord, defineStatus).
          } catch (e) {
            return IteratorClose(iteratorRecord, e);
          } // x. Increase k by 1.
      ...
      var len = ToLength(Get(arrayLike, "length")); // 9. If IsConstructor(C) is true, then

      if (IsConstructor(C)) {
        // a. Let A be ? Construct(C, « len »).
        A = Construct(C, [len]); // 10. Else,
      } else {
      ...
      k = 0; // 12. Repeat, while k < len

      while (k < len) {
        // a. Let Pk be ! ToString(k).
        Pk = ToString(k); // b. Let kValue be ? Get(arrayLike, Pk).

        var kValue = Get(arrayLike, Pk); // c. If mapping is true, then

        if (mapping === true) {
          // i. Let mappedValue be ? Call(mapfn, T, « kValue, k »).
          mappedValue = Call(mapfn, T, [kValue, k]); // d. Else, let mappedValue be kValue.
        } else {
          mappedValue = kValue;
        } // e. Perform ? CreateDataPropertyOrThrow(A, Pk, mappedValue).


        CreateDataPropertyOrThrow(A, Pk, mappedValue); // f. Increase k by 1.

        k = k + 1;
      } // 13. Perform ? Set(A, "length", len, true).


      A.length = len; // 14. Return A.

      return A;
    });
  })();
})('object' === (typeof window === "undefined" ? "undefined" : _typeof2(window)) && window || 'object' === (typeof self === "undefined" ? "undefined" : _typeof2(self)) && self || 'object' === (typeof global === "undefined" ? "undefined" : _typeof2(global)) && global || {});
